...
# This is my first R scripting; so it is verbose as I am a novice. I will, 
# with consistent practice, write efficient and compact code for data analysis.
...
data <- read_sav("data/covid-19.sav")
...
result <- data %>%
  filter(WeightChnge > 0) %>%  
  summarize(
    mean_value = mean(WeightChnge, na.rm = TRUE),  
    sd_value = sd(WeightChnge, na.rm = TRUE)      
  )
...
increase_weight <- increase_weight %>%
  mutate(Category = "Increase")

decrease_weight <- data.frame(summary_decrease_weight)

decrease_weight <- decrease_weight %>%
  mutate(Category = "Decrease")

increase_weight <- increase_weight %>%
  rename(
    mean_weight = mean_weight_increase,
    sd_weight = sd_weight_increase,
    se_weight = se_weight_increase
  )

decrease_weight <- decrease_weight %>%
  rename(
    mean_weight = mean_weight_decrease,
    sd_weight = sd_weight_decrease,
    se_weight = se_weight_decrease
  )

summary_weight <- bind_rows(increase_weight, decrease_weight)
